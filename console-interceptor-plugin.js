 ...
 */
class ConsoleInterceptorPlugin extends BasePlugin {
  constructor(config = {}) {
    const defaultConfig = {
      enabled: true,
      levels: ['log', 'error', 'warn', 'info'], // Console methods to intercept
  ...
  forwardLogEvent(level, args) {
    ...
    if (this.pluginManager) {
      setImmediate(() => {
        this.pluginManager.callHook('onLog', 
          logData.level, 
          logData.message, 
          logData, 
...
module.exports = { ConsoleInterceptorPlugin };