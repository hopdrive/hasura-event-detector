...
class EventModuleBuilder {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (entry.name === 'events') {
            // Find all .ts files in this events directory
  ...
  compileFile(tsFilePath) {
    ...
    // Rename .d.ts to .generated.d.ts
    const dtsFilePath = path.join(dir, `${baseName}.d.ts`);
    const generatedDtsFilePath = path.join(dir, `${baseName}.generated.d.ts`);
    if (fs.existsSync(dtsFilePath)) {
      fs.renameSync(dtsFilePath, generatedDtsFilePath);
    }
